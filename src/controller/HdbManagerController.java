...
public class HdbManagerController {
    ...
    public String generateBookingReport(HdbManager manager, String filterProjectName, MaritalStatus filterMaritalStatus, FlatType filterFlatType) {
        ...
        if (filterFlatType != null) {
            // This requires the booked flat type to be stored on the ProjectApplication object
            // filteredStream = filteredStream.filter(app -> filterFlatType.equals(app.getBookedFlatType()));
            System.out.println("WARNING: Filtering by booked flat type requires storing it on the application (feature not implemented).");
        }

        List<ProjectApplication> reportApps = filteredStream
                .sorted(Comparator.comparing(ProjectApplication::getProjectName) // Sort by project then applicant
        ...
        report.append(" | Booked Flat: ").append(filterFlatType != null ? filterFlatType : "ALL (Filter not implemented)");
        report.append("\n------------------------------------------\n");
        ...
        if (reportApps.isEmpty()) {
            ...
            for (ProjectApplication app : reportApps) {
                 ...
                 // TODO: Get booked flat type from application object when available
                 String bookedTypeStr = "N/A"; // Placeholder = app.getBookedFlatType().name();
